...
echo "****************Inputdata Download started...****************"

sh inputdata_download.sh $1 $2
echo "****************Inputdata Download completed...****************"

echo "************************************************ RUN ID *******************************************************"
cd /apps/data/cmrods/input/inputData
filename=`ls saccr_report_OSFI-BFG-NEW_*_${2}_*.csv | tail -1`
echo "Using: $filename"
runID=`echo ${filename} |cut -d'_' -f6 | cut -d'.' -f1`
echo "*********** Run ID = $runID ****************"
